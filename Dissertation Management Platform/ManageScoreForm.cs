...
namespace TexnologiaLogismikou
{
    public partial class ManageScoreForm : Form
    {
        ...
        private void ManageScoreForm_Load(object sender, EventArgs e)
        {
            ...
            // to show score data on datagridview table
            showScore();
        }

        public void showScore()
        {
            //maybe course instead of score
        ...
        {
            if (textBox_stdId.Text == "" || textBox_score.Text == "") //both should be filled to update score successfully
            {
                MessageBox.Show("Τα στοιχεία, ώστε να μπει ο βαθμός, είναι ελλιπή.", "Σφάλμα", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            else
            {
                ...
                string desc = textBox_description.Text;
                    ...
                    if (score.updateScore(stdId, cName, scor, desc))
                    {
                        ...
                        MessageBox.Show("Επικαιροποιήθηκε o βαθμός!", "Επικαιροποίηση βαθμού", MessageBoxButtons.OK, MessageBoxIcon.Information);

                    }
                    else
                    {
                        MessageBox.Show("Δεν επικαιροποιήθηκε ο βαθμός.", "Επικαιροποίηση βαθμού", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }            
            }
        }
        ...
        private void button_delete_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Σφάλμα. Χρειαζόμαστε το ID του Φοιτητή.", "Διαγραφή Βαθμού", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            else
            {
                int id = Convert.ToInt32(textBox_stdId.Text);
                if (MessageBox.Show("Θέλετε σίγουρα να αφαιρέσετε αυτόν τον βαθμό;", "Διαγραφή Βαθμού", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    if (score.deleteScore(id))
                    {
                        showScore();
                        MessageBox.Show("Ο βαθμός διαγράφηκε.", "Διαγραφή Βαθμού", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        button_clear.PerformClick();
                    }
                }
